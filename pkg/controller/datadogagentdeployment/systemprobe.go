...
const (
	// SystemProbeConfigMapName SystemProbe Config configmap name
	SystemProbeConfigMapName = "system-probe-config"
	// SystemProbeAgentSecurityConfigMapName AgentSecurity configmap name
	SystemProbeAgentSecurityConfigMapName = "datadog-agent-security"
)
...
func (r *ReconcileDatadogAgentDeployment) cleanupSystemProbeDependencies(logger logr.Logger, dad *datadoghqv1alpha1.DatadogAgentDeployment) (reconcile.Result, error) {
	result, err := r.cleanupConfigMap(logger, dad, SystemProbeConfigMapName)
	if shouldReturn(result, err) {
		return result, err
	}
	result, err = r.cleanupConfigMap(logger, dad, SystemProbeAgentSecurityConfigMapName)
	return result, err
}
...
func buildSystemProbeSecCompConfigMap(dad *datadoghqv1alpha1.DatadogAgentDeployment) (*corev1.ConfigMap, error) {
	configMap := &corev1.ConfigMap{
		ObjectMeta: metav1.ObjectMeta{
			Name:        SystemProbeAgentSecurityConfigMapName,
			Namespace:   dad.Namespace,
			Labels:      getDefaultLabels(dad, dad.Name, getAgentVersion(dad)),
			Annotations: getDefaultAnnotations(dad),
		},
